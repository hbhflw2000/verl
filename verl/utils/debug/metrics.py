...
def _maybe_log_rollout_corr_debug(
    ...
    if ref_log_probs is not None:
        print(f"[RolloutCorrDebug] {_summary('ref_log_prob', ref_log_probs)}")
        print(f"[RolloutCorrDebug] {_summary('old_minus_ref', actor_old_log_probs - ref_log_probs)}")
        print(f"[RolloutCorrDebug] {_summary('ref_minus_rollout', ref_log_probs - rollout_old_log_probs)}")
    ...
    if (
        ...
        include_ref = ref_log_probs is not None and ref_log_probs.shape == actor_old_log_probs.shape
        for row_idx in range(row_count):
            row_pairs = []
            for col_idx in range(token_count):
                ref_payload = {}
                if include_ref:
                    ref_value = ref_log_probs[row_idx, col_idx]
                    ref_payload = {
                        "ref": round(float(ref_value.item()), 6),
                        "old_ref_diff": round(float((actor_old_log_probs[row_idx, col_idx] - ref_value).item()), 6),
                        "ref_rollout_diff": round(float((ref_value - rollout_old_log_probs[row_idx, col_idx]).item()), 6),
                    }
                row_pairs.append(
                    {
                        "pos": col_idx,
...
def _to_cpu_list(tensor: torch.Tensor) -> list:
    return tensor.detach().cpu().tolist()


def _maybe_dump_rollout_corr_samples(
    data: DataProto,
    actor_old_log_probs: torch.Tensor,
    rollout_old_log_probs: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    output_path = os.environ.get("VERL_OMNI_ROLLOUT_CORR_DEBUG_JSONL", "")
    if not output_path:
        return

    try:
        row_limit = int(os.environ.get("VERL_OMNI_ROLLOUT_CORR_DEBUG_JSONL_ROWS", "4"))
    except ValueError:
        row_limit = 4
    if row_limit <= 0:
        return
    ...
    batch = data.batch
    required = ("input_ids", "responses")
    missing = [key for key in required if key not in batch]
    if missing:
        print(f"[RolloutCorrDebug] skip jsonl dump, missing keys={missing}")
        return
    ...
    if (
        actor_old_log_probs.shape != rollout_old_log_probs.shape
        or actor_old_log_probs.shape != response_mask.shape
        or responses.shape != response_mask.shape
        or input_ids.ndim != 2
    ):
        print(
            "[RolloutCorrDebug] skip jsonl dump due to shape mismatch "
            f"input_ids={tuple(input_ids.shape)} responses={tuple(responses.shape)} "
            f"old={tuple(actor_old_log_probs.shape)} rollout={tuple(rollout_old_log_probs.shape)} "
    ...
    row_count = min(row_limit, input_ids.size(0))
    records = []
    for row_idx in range(row_count):
        record = {
            "event": "rollout_corr_sample",
            ...
            "shapes": {
                ...
                "actor_old_log_probs": list(actor_old_log_probs.shape),
                "rollout_log_probs": list(rollout_old_log_probs.shape),
                "ref_log_probs": None if ref_log_probs is None else list(ref_log_probs.shape),
                "prompts": None if prompts is None else list(prompts.shape),
                "position_ids": None if position_ids is None else list(position_ids.shape),
            },
            "input_ids": _to_cpu_list(input_ids[row_idx]),
            "attention_mask": _to_cpu_list(attention_mask[row_idx]),
            "responses": _to_cpu_list(responses[row_idx]),
            "response_mask": _to_cpu_list(response_mask[row_idx]),
            "actor_old_log_probs": _to_cpu_list(actor_old_log_probs[row_idx].float()),
            "rollout_log_probs": _to_cpu_list(rollout_old_log_probs[row_idx].float()),
        }
        if ref_log_probs is not None and ref_log_probs.shape == actor_old_log_probs.shape:
            record["ref_log_probs"] = _to_cpu_list(ref_log_probs[row_idx].float())
        if prompts is not None and prompts.ndim >= 2:
            record["prompts"] = _to_cpu_list(prompts[row_idx])
        if position_ids is not None:
            # Full position_ids can be 3D for multimodal models; keep enough to verify
            # contract without making every JSONL record huge.
            pos_row = position_ids[row_idx]
            record["position_ids_shape"] = list(pos_row.shape)
            if pos_row.numel() <= input_ids.size(1) * 4:
                record["position_ids"] = _to_cpu_list(pos_row)
        records.append(record)

    with open(output_path, "a", encoding="utf-8") as f:
        for record in records:
            f.write(json.dumps(record, ensure_ascii=False, sort_keys=True) + "\n")
    print(f"[RolloutCorrDebug] dumped {len(records)} fixed samples to {output_path}")
...
def calculate_debug_metrics(data: DataProto) -> dict:
    ...
    try:
        _maybe_dump_rollout_corr_samples(
            data,
            actor_old_log_probs,
            rollout_old_log_probs,
            response_mask,
        ...
        print(f"[RolloutCorrDebug] failed to dump rollout corr debug jsonl: {exc}")
    ...
    if ref_log_probs is not None:
        ...
        metrics.update(_masked_value_stats("training/ref_log_prob", ref_log_probs, response_mask_bool))
        metrics.update(
            {
                "training/actor_ref_logprob_pearson_corr": _masked_corrcoef(
                    actor_old_log_probs, ref_log_probs, response_mask_bool
                ),
                "training/actor_ref_logprob_abs_diff_mean": _masked_mean_abs_diff(
                    actor_old_log_probs, ref_log_probs, response_mask_bool
                ),
                "training/rollout_ref_logprob_pearson_corr": _masked_corrcoef(
                    rollout_old_log_probs, ref_log_probs, response_mask_bool
                ),
                "training/rollout_ref_logprob_abs_diff_mean": _masked_mean_abs_diff(
                    rollout_old_log_probs, ref_log_probs, response_mask_bool
                ),
                "training/actor_ref_probs_pearson_corr": pearson_correlation_coefficient(
                    actor_probs, ref_probs, response_mask_bool
                ),
                "training/rollout_ref_probs_pearson_corr": pearson_correlation_coefficient(
                    rollout_probs, ref_probs, response_mask_bool
                ),
            }
        )
    return metrics
